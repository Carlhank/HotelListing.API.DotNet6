...
public class AuthManager : IAuthManager
{
    ...
    public async Task<string> CreateRefreshToken()
    {
        ...
        return result.Succeeded ? newFreshToken : null;
    }
    ...
    public async Task<AuthResponseDto> VerifyRefreshToken(AuthResponseDto request)
    {
        ...
        var username = Enumerable.ToList<Claim>(tokenContent.Claims)
    ...
    {
        var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes((string)_configuration["JwtSettings:Key"]));
        var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);
        
        var roles = await _userManager.GetRolesAsync(_user);
        var roleClaims = Enumerable.Select<string, Claim>(roles, role => new Claim(ClaimTypes.Role, role)).ToList();
        ...
        var token = new JwtSecurityToken(
            ...
            claims: claims,
            expires: DateTime.Now.AddMinutes(Convert.ToInt32((string?)_configuration["JwtSettings:DurationInMinutes"])),
